/*
 * extra_platform_init.c
 *
 *  Created on: 2014-4-22
 *      Author: Administrator
 */
#include "extra_platform_init.h"
int toggle = 0;//used to toggle the LED

/**
 * The function init the platform and some interrupt.
 */
void ExtraPlatformInit(void)
{
	//print the initial tips
//	printf("\n\rextra_platform_init called \n\r");

	//init the platform
	init_platform();

	//config definiation
//	XAdcPs_Config *ConfigPtr;           //xadc config
	XGpioPs_Config *GPIOConfigPtr;      //gpio config
	XScuTimer_Config *TMRConfigPtr;     //timer config
	XScuWdt_Config *WCHConfigPtr;		//watchdog config
//	XAdcPs *XADCInstPtr = &XADCMonInst; //xadc pointer

	//UART Init
//	UART_Init(&XUart);

	int Status;
	//GPIO Initilization
	GPIOConfigPtr = XGpioPs_LookupConfig(XPAR_XGPIOPS_0_DEVICE_ID);
	Status = XGpioPs_CfgInitialize(&Gpio, GPIOConfigPtr,GPIOConfigPtr->BaseAddr);
	if (Status != XST_SUCCESS) {
//	    		 print("GPIO INIT FAILED\n\r");
	    		return XST_FAILURE;
	}
	//set direction and enable output
	XGpioPs_SetDirectionPin(&Gpio, ledpin, 1);
	XGpioPs_SetOutputEnablePin(&Gpio, ledpin, 1);
	//set direction input pin
	XGpioPs_SetDirectionPin(&Gpio, pbsw, 0x0);

	//timer initialisation
	TMRConfigPtr = XScuTimer_LookupConfig(TIMER_DEVICE_ID);
	XScuTimer_CfgInitialize(&Timer, TMRConfigPtr,TMRConfigPtr->BaseAddr);
	XScuTimer_SelfTest(&Timer);
	//load the timer
	XScuTimer_LoadTimer(&Timer, TIMER_LOAD_VALUE);
	XScuTimer_EnableAutoReload(&Timer);

	//watchdog configuration
	WCHConfigPtr = XScuWdt_LookupConfig(WDT_DEVICE_ID);
	XScuWdt_CfgInitialize(&Wdt, WCHConfigPtr,WCHConfigPtr->BaseAddr);
	XScuWdt_LoadWdt(&Wdt, WDT_LOAD_VALUE);
	XScuWdt_SetWdMode(&Wdt);

	//get the last reboot info
	u32 state = XScuWdt_IsWdtExpired(&Wdt);
	if(state == TRUE)
		UART_Fail(TEST_TYPE_WDTREBOOT, NULL, 0);
//	printf("\n\rReset state is %ld \n\r", state);

	/*//XADC initilization
	ConfigPtr = XAdcPs_LookupConfig(XPAR_AXI_XADC_0_DEVICE_ID);
	if (ConfigPtr == NULL) {
//		print("ADC INIT FAILED\n\r");
		return XST_FAILURE;
	}
	//adc set up
	int Status_ADC;
	Status_ADC = XAdcPs_CfgInitialize(XADCInstPtr,ConfigPtr,ConfigPtr->BaseAddress);
	if(XST_SUCCESS != Status_ADC){
//		print("ADC INIT FAILED\n\r");
		return XST_FAILURE;
	}
	//self test adc
	Status_ADC = XAdcPs_SelfTest(XADCInstPtr);
	if (Status_ADC != XST_SUCCESS) {
//		print("ADC INIT FAILED\n\r");
		return XST_FAILURE;
	}
	//stop sequencer
	XAdcPs_SetSequencerMode(XADCInstPtr,XADCPS_SEQ_MODE_SINGCHAN);
	//disable alarms
	XAdcPs_SetAlarmEnables(XADCInstPtr, 0x0);
	//configure sequencer to just sample internal on chip parameters
	XAdcPs_SetSeqInputMode(XADCInstPtr, XADCPS_SEQ_MODE_SAFE);
	//configure the channel enables we want to monitor
	XAdcPs_SetSeqChEnables(XADCInstPtr,XADCPS_CH_TEMP|XADCPS_CH_VCCINT|XADCPS_CH_VCCAUX|XADCPS_CH_VBRAM|XADCPS_CH_VCCPINT|
	     		XADCPS_CH_VCCPAUX|XADCPS_CH_VCCPDRO);*/

	//set up the interrupts
	SetupInterruptSystem(&Intc, &Gpio, GPIO_INTERRUPT_ID,&Timer,&Wdt,TIMER_IRPT_INTR,WDT_IRPT_INTR);

	XScuWdt_Start(&Wdt);//start the WatchDog
}

/**
 * Interrupt Initializer
 */
void SetupInterruptSystem(XScuGic *GicInstancePtr, XGpioPs *Gpio, u16 GpioIntrId,
		XScuTimer *TimerInstancePtr, XScuWdt *WdtInstancePtr, u16 TimerIntrId, u16 WdtIntrId)
{
		XScuGic_Config *IntcConfig; //GIC config
		Xil_ExceptionInit();
		//initialise the GIC
		IntcConfig = XScuGic_LookupConfig(INTC_DEVICE_ID);
		XScuGic_CfgInitialize(GicInstancePtr, IntcConfig,
						IntcConfig->CpuBaseAddress);
	    //connect to the hardware
		Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_INT,
					(Xil_ExceptionHandler)XScuGic_InterruptHandler,
					GicInstancePtr);
	    //set up the GPIO interrupt
		XScuGic_Connect(GicInstancePtr, GpioIntrId,
					(Xil_ExceptionHandler)XGpioPs_IntrHandler,
					(void *)Gpio);
		//set up the timer interrupt
		XScuGic_Connect(GicInstancePtr, TimerIntrId,
						(Xil_ExceptionHandler)TimerIntrHandler,
						(void *)TimerInstancePtr);
		//set up the watchdog
		XScuGic_Connect(GicInstancePtr, WdtIntrId
					, (Xil_ExceptionHandler)WdtIntrHandle
					, (void *)WdtInstancePtr);
		//Enable  interrupts for all the pins in bank 0.
		XGpioPs_SetIntrTypePin(Gpio, pbsw, XGPIOPS_IRQ_TYPE_EDGE_RISING);
		//Set the handler for gpio interrupts.
		XGpioPs_SetCallbackHandler(Gpio, (void *)Gpio, GPIOIntrHandler);
		//Enable the GPIO interrupts of Bank 0.
		XGpioPs_IntrEnablePin(Gpio, pbsw);
		//Enable the interrupt for the GPIO device.
//		XScuGic_Enable(GicInstancePtr, GpioIntrId);
		//enable the interrupt for the Timer at GIC
		XScuGic_Enable(GicInstancePtr, TimerIntrId);
		//enable interrupt on the timer
		XScuTimer_EnableInterrupt(TimerInstancePtr);
		// Enable interrupts in the Processor.
		Xil_ExceptionEnableMask(XIL_EXCEPTION_IRQ);
}

/**
 * Timer Int. Handler
 */
void TimerIntrHandler(void *CallBackRef)
{
	XScuWdt_LoadWdt(&Wdt, WDT_LOAD_VALUE);
	XScuTimer *TimerInstancePtr = (XScuTimer *) CallBackRef;
	XScuTimer_ClearInterruptStatus(TimerInstancePtr);
//	XScuTimer_Stop(TimerInstancePtr);
//	XScuGic_Enable(&Gpio, GPIO_INTERRUPT_ID);
//	printf("****Timer Event!!!!!!!!!!!!!****\n\r");
//	char bc = 8;
//	printf("\n%d", --nTimeRemain);
//	printf("%d \n\r",XScuTimer_GetCounterValue(TimerInstancePtr));
	/*if(nTimeRemain == 0)
	{
		while(nStartTestFlag)
		{
			XScuWdt_LoadWdt(&Wdt, WDT_LOAD_VALUE);
			TestAll();
			Delay();
	//		XScuWdt_WriteReg(XPAR_SCUWDT_0_BASEADDR, XSCUWDT_LOAD_OFFSET, WDT_LOAD_VALUE);
		}
	}*/
}

/**
 * Watchdog Int. Handler
 */
void WdtIntrHandle(void *CallBackRef)
{
//	printf("死掉，开始重启\n");
}


/**
 * GPIO Int. Handler
 */
void GPIOIntrHandler(void *CallBackRef, int Bank, u32 Status)
{
	XGpioPs *Gpioint = (XGpioPs *)CallBackRef;

//	printf("****button pressed****\n\r");

	if (toggle == 0 )
	{
		toggle = 1;
	}
	else
	{
		toggle = 0;
	}

//	test_type = (++test_type) % 6;
	nStartTestFlag = nStartTestFlag == 1 ? 0 : 1;

	//load timer
//	XScuTimer_LoadTimer(&Timer, TIMER_LOAD_VALUE);
	//start timer
//	XScuTimer_Start(&Timer);
	XGpioPs_WritePin(Gpioint, ledpin, toggle);

	Delay();

	XGpioPs_IntrClearPin(Gpioint, pbsw);
}
